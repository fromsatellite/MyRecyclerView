package com.dongnao.myrecyclerview;
...
//VIEW 打tag   row    type
        ...
        // TODO 此处的index为什么一直是0
        addView(view,0);
        return view;
    }

    /**
     * 从数组array[]中取出count个元素，起始位置是firstIndex,则要取的最后一个元素下标是firstIndex+count-1。
     * <p>要注意count参数的取值，firstIndex+count是取不到的。<p/>
     * @param array
     *          源数组。
     * @param firstIndex
     *          开始位置(结果中的第一个元素)。
...
//
//                int initY = scrollY + sumArray(heights, 0, firstRow);
...
//              remove  item完全移出去了  应该 1  不应该2
            ...
            //            scrollY=0
            int filledH = getFilledHeight();
            while (filledH<height){
                addBottom();
                filledH = getFilledHeight();
            }

        } else if (scrollY < 0) {
            //            往下滑
            int filledH = getFilledHeight();
            while (!viewList.isEmpty() && filledH - heights[firstRow + viewList.size()] > height) {
                removeBottom();
                filledH = getFilledHeight();
            }
...
//        重绘
    ...
    private int scrollBounds(int scrollY, int firstRow, int sizes[], int viewSize) {
        if (scrollY > 0) {
            // 往上滑  bug +，往上滑时最后一个item不会离开屏幕底部，所以要-viewSize
            //
